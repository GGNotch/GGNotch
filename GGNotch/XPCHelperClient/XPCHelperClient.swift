...
final class XPCHelperClient: NSObject {
    ...
    private let serviceName = "com.ggnotch.app.GGNotchXPCHelper"
    
    private var remoteService: RemoteXPCService<GGNotchXPCHelperProtocol>?
    private var connection: NSXPCConnection?
    private var lastKnownAuthorization: Bool?
    private var monitoringTask: Task<Void, Never>?
    private var lunarListener: GGNotchXPCHelperLunarListener?
    ...
    @MainActor
    private func ensureRemoteService(needsListener: Bool = false) -> RemoteXPCService<GGNotchXPCHelperProtocol> {
        ...
        let service = RemoteXPCService<GGNotchXPCHelperProtocol>(
            connection: conn,
            remoteInterface: GGNotchXPCHelperProtocol.self
        )
    ...
    @MainActor
    private func getRemoteService() -> RemoteXPCService<GGNotchXPCHelperProtocol>? {
        remoteService
    }

    private func makeLunarListenerInterface() -> NSXPCInterface {
        let interface = NSXPCInterface(with: (any GGNotchXPCHelperLunarListener).self)
        interface.setClasses(
            NSSet(array: [BNLunarBrightnessEvent.self]) as! Set<AnyHashable>,
            for: #selector(GGNotchXPCHelperLunarListener.lunarEventDidUpdate(_:)),
            argumentIndex: 0,
            ofReply: false
        )
        return interface
    ...
    nonisolated func startLunarEventStream(listener: GGNotchXPCHelperLunarListener) async -> Bool {
        await MainActor.run {
