    ...
    private func showOSD(for keyType: NXKeyType, command: Bool) {
        Task { @MainActor in
            ...
            case .soundUp, .soundDown, .mute:
                ...
                GGNotchViewCoordinator.shared.toggleSneakPeek(status: true, type: .volume, value: CGFloat(v))
            case .brightnessUp, .brightnessDown:
                if command {
                    let v = KeyboardBacklightManager.shared.rawBrightness
                    GGNotchViewCoordinator.shared.toggleSneakPeek(status: true, type: .backlight, value: CGFloat(v))
                } else {
                    let v = BrightnessManager.shared.rawBrightness
                    Task { @MainActor in
                        let target = await BrightnessManager.shared.brightnessTargetUUID()
                        GGNotchViewCoordinator.shared.toggleSneakPeek(status: true, type: .brightness, value: CGFloat(v), targetScreenUUID: target)
                    }
                }
            case .keyboardBrightnessUp, .keyboardBrightnessDown:
                let v = KeyboardBacklightManager.shared.rawBrightness
