	...
	@MainActor func setRelative(delta: Float) {
		Task { @MainActor in
			...
			if ok {
                ...
                GGNotchViewCoordinator.shared.toggleSneakPeek(status: true, type: .brightness, value: CGFloat(current), targetScreenUUID: targetUUID)
			} else {
				refresh()
			}
		}
	...
	func setAbsolute(value: Float) {
		...
		Task { @MainActor in
			...
			if ok {
                ...
                let targetUUID = await brightnessTargetUUID()
                GGNotchViewCoordinator.shared.toggleSneakPeek(status: true, type: .brightness, value: CGFloat(clamped), targetScreenUUID: targetUUID)
			} else {
				refresh()
			}
		}
...
final class KeyboardBacklightManager: ObservableObject {
	...
	@MainActor func setRelative(delta: Float) {
		Task { @MainActor in
			...
			GGNotchViewCoordinator.shared.toggleSneakPeek(
				status: true,
				type: .backlight,
