...
final class BetterDisplayManager {
    ...
    private func handleOsdNotification(_ notification: Notification) async {
        ...
        case .volume:
            ...
            await MainActor.run {
                GGNotchViewCoordinator.shared.toggleSneakPeek(status: true, type: .volume, value: CGFloat(normalized))
            }

        case .mute:
            await MainActor.run {
                GGNotchViewCoordinator.shared.toggleSneakPeek(status: true, type: .volume, value: CGFloat(rawValue))
            }
