...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("GGNotch", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings") {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var viewModels: [String: GGNotchViewModel] = [:] // UUID -> GGNotchViewModel
    var window: NSWindow?
    let vm: GGNotchViewModel = .init()
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? GGNotchSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? GGNotchSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createGGNotchWindow(for screen: NSScreen, with viewModel: GGNotchViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = GGNotchSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createGGNotchWindow(
    ...
    func playWelcomeSound() {
        ...
        audioPlayer.play(fileName: "ggNotch", fileExtension: "m4a")
    }
    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = GGNotchViewModel(screenUUID: uuid)
                    let window = createGGNotchWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createGGNotchWindow(for: selectedScreen, with: vm)
            }
